<template>
  <div class="context-editor-content context-editor-component">
    <div v-if="errors['/context']" class="alert alert-danger py-1 small mb-2">
      {{ errors['/context'].join(', ') }}
    </div>
    <div class="mb-3">
      <label for="context-id" class="form-label">ID</label>
      <input type="text" class="form-control input-context-id" :class="{'is-invalid': errors['/context/id']}" id="context-id" :value="safeContext.id" @input="update('id', $event.target.value)">
    ...
    <div class="mb-3">
      ...
      <div class="form-check">
        <input class="form-check-input radio-context-type-plans" :class="{'is-invalid': errors['/context/type']}" type="radio" name="context-type" id="type-plans" value="plans" :checked="safeContext.type === 'plans'" @change="update('type', 'plans')">
      ...
      <div class="form-check mt-2">
        <input class="form-check-input radio-context-type-agreements" :class="{'is-invalid': errors['/context/type']}" type="radio" name="context-type" id="type-agreements" value="agreements" :checked="safeContext.type === 'agreements'" @change="update('type', 'agreements')">
