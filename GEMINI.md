...
### Source Control & Build Artifacts

*   **No Generated Files:** No files generated by the build system (e.g., ANTLR4 parser files, distribution bundles) should ever be part of the source control. Ensure they are correctly listed in `.gitignore` and `.dockerignore`.
...
*   **Terraform Syntax (Escaping):** Complex Terraform attributes like monitoring filters require nested quote escaping. In JavaScript/Vue templates, this often means using triple backslashes (`\\\\\\`) to represent a single literal backslash in the final `.tf` file if using string concatenation. However, using a structured Code Emitter significantly reduces these escaping errors.

### Lessons Learned (Session: Terraform Parser & Code Emitter)

*   **Structured Emitter vs. String Concatenation:** Moving from manual string templates to a structured AST-based code emitter for Terraform (and PromQL) drastically improves code reliability and reduces escaping bugs (e.g., handling quotes in labels or multiline strings with heredocs).
*   **ANTLR4 ESM Imports:** When using `antlr4ts` for web projects (Vite), generated files often require post-processing (e.g., via `sed`) to add `.js` extensions to local imports and reference `antlr4ts/index.js` to comply with ESM standards.
*   **Build-Generated Artifacts:** Rigorously maintain `.gitignore` to ensure that artifacts generated by the ANTLR4 build step (e.g., `*Parser.ts`, `*Lexer.ts`, `.interp`, `.tokens`) are never committed to the repository. The build system should regenerate these from the `.g4` grammar files on every build.